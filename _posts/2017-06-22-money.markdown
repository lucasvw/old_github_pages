...
- Since there are $$m - 1$$ other players, the probability that the dollar from one specific other player ends up with player $$A$$ (in a single round) equals $$p = 1/(m-1)$$.
...
Now we can draw $$m$$ samples from this distribution, one for each player of the game in a single round. This sample thus denotes the amount of money each single player receives in this round. Naturally, the money that is spend each round, should equal the amount of money that is received each round. However, because the amount that is received is now stochastic (sampled from a binomial distribution) there is no guarentee that this is the case. Nonetheless, we know the mean of the binomial distribution equals $$np = (m-1)*\frac{1}{m-1} = 1$$. So each player is expected to gain a single dollar each round. Because this is also the amount each person gives away, the *expected* amount of money in the game will remain constant. For now, I am quite okay with this assumption.
...
Now the thing is, that (in hindsight admittedly..), I don't think that the result is actually so counter-intuitive. And for that we will make a de-tour through coin-flipping. Consider a fair coin so that the change of getting heads for a single flip is $$\frac{1}{2}$$. Now, if I flip the coin 10 times, I *expect* to see 5 ( = $$0.5 * 10$$) heads. Let's do this a couple of times and note the amount of heads we see after 10 flips:
4
6
5
4
